...
//
// The country picker stores the alpha-2 code (from lib/countries), but the
// backend expects the alpha-3 ISO code (e.g. "TZA"). The backend's
// /v1/lookup/countries can return an empty list, so we resolve codes locally
...
/** Convert an alpha-2 code (e.g. "TZ") to alpha-3 ("TZA"), or null if unknown. */
